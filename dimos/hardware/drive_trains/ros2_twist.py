"""Fail-closed DimOS Twist output for ROS 2 chassis controllers."""
...
class ROS2TwistConnectionConfig(ModuleConfig):
    enabled: bool = False
    ros_cmd_vel_topic: str = ""
    ros_node_name: str = "dimos_ros2_twist"
    ...
    def validate_output(self) -> ROS2TwistConnectionConfig:
        if self.enabled and not self.ros_cmd_vel_topic.startswith("/"):
            raise ValueError("enabled ROS Twist output requires an absolute topic")
...
class ROS2TwistConnection(Module):
    """Publish bounded commands to a platform-owned ROS 2 Twist endpoint."""
    ...
    async def main(self) -> AsyncGenerator[None, None]:
        if self.config.enabled:
            ...
            logger.warning(
                "ROS motion output enabled: topic=%s linear=%s lateral=%s angular=%s",
                self.config.ros_cmd_vel_topic,
                self.config.max_linear_mps,
                self.config.max_lateral_mps,
                self.config.max_angular_radps,
            )
        else:
            logger.warning("ROS motion output disabled; no publisher created")
        try:
            yield
        finally:
    ...
    def _start_ros_publisher(self) -> None:
        try:
            ...
            raise RuntimeError("ROS2TwistConnection requires ROS 2 Python packages") from exc
        if not rclpy.ok():
            rclpy.init(args=None)
        self._node = Node(self.config.ros_node_name)
        self._ros_twist_type = RosTwist
        self._publisher = self._node.create_publisher(
            RosTwist, self.config.ros_cmd_vel_topic, 1
        )
    ...
    def _bounded(self, msg: Twist) -> Twist:
        lateral = (
            self._finite_and_clamp(msg.linear.y, self.config.max_lateral_mps)
            if self.config.allow_lateral
            else 0.0
        )
        return Twist(
            linear=[
                self._finite_and_clamp(msg.linear.x, self.config.max_linear_mps),
                lateral,
                0.0,
            ],
            angular=[
                0.0,
                0.0,
                self._finite_and_clamp(msg.angular.z, self.config.max_angular_radps),
            ],
        )
