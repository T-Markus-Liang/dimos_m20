...
class TestHERTABMapRuntimeBounds(unittest.TestCase):
    ...
    def test_shadow_systemd_mode_is_bounded_static_and_restorable(self) -> None:
        ...
        storage_unit = (DEPLOYMENT_DIR / "he-storage-health.service").read_text()
        aurora_storage = (DEPLOYMENT_DIR / "aurora930-storage-health.conf").read_text()
        switch = (DEPLOYMENT_DIR / "switch-he-dimos-mode.sh").read_text()
        sense_gate = (DEPLOYMENT_DIR / "verify-he-readonly.sh").read_text()
        ...
        for unit in (sense_unit, shadow_unit, throttle_unit):
            self.assertIn("Requires=he-storage-health.service", unit)
